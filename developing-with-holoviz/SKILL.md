...
| [exploring](skills/exploring/SKILL.md) | Profile any multidimensional dataset (column roles → measures/dimensions/time/ids), then write a grounded narrative EDA notebook (percent-script) that climbs the ladder and is **interactive by default** — inline `groupby`/HoloMap widgets where a view has a free dimension, a small one-control-drives-every-panel Viewer for linked views — pushing heavy aggregation to the engine (DuckDB) |
