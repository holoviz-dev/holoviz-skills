...
| Explore, profile, or understand an unfamiliar dataset; EDA; "what's in this data"; a narrative or interactive data story | `developing-with-holoviz/skills/exploring/SKILL.md` |
...
| [exploring](skills/exploring/SKILL.md) | Profile any multidimensional dataset (column roles → measures/dimensions/time/ids), then build a grounded, narrative EDA — a notebook (percent-script) or a servable Panel layout you can render — climbing the ladder one question at a time and **interactive by default**: `groupby=` knobs where a view has a free dimension, DynamicMap + pn.bind for deeper one-control views, with heavy aggregation pushed to the engine (DuckDB) |
