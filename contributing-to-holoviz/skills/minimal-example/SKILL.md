...
- Import every submodule the snippet uses explicitly depending on the module, e.g.
  `from holoviews.operation.datashader import datashade` rather than reaching for
  `hv.operation.datashader`. Submodule attribute access only resolves once
  something has already imported it, so a snippet written in a warm session runs
  for you and raises `AttributeError` for the first person to paste it cold.
- Check the finished snippet in a fresh interpreter, not the session you wrote it
  in and not against your patched checkout. A session carries imports, registered
  extensions and monkeypatches that the reader's won't, and a local dev install
  reports its own version, so the `__version__` you print should be the one you
  are reporting against.
...
- For a bug, paste the versions that matter (`hv.__version__`, `pn.__version__`,
  ...
  version-specific.
- Say where the before/after shows up. `pn.serve` blocks and renders in a
  browser, so a visual symptom is only visible there, while an exception or a
  warning lands in the terminal. A tester watching the wrong one concludes
  nothing happened.
